...
def get_parameters():
	...
	print(color, "Ye kia chal raha hai")
...
def straight(position, smooth=True):
	corner = bot()
	...
	kd = 0
	if not smooth:
		diff = x_point(position) - center_of(bot())[index]
		diff *= unit
		v = kp*diff
		while diff > 0:
			run = 1
...
def right(smooth="False"):
	...
	ki = 80
	kd = 0
	diff = bot()[0][index] - bot()[1][index]
	diff *= unit
	v = kp*diff
	interval = int(ki*diff)
	run = 1
	while diff > 0:
		if interval == 0:
			break
		if interval != 0:
			...
			rev = int(interval*1.0173)
			run = 1
			while run%rev:
				p.stepSimulation()
				env.move_husky(0, -v, 0, -v)
				run += 1
	...
	if smooth:
		straight(position_of_bot())

def left(smooth="False"):
	corner = bot()
	if math.isclose(corner[0][0], corner[1][0], abs_tol = 3):
		index = 1
	else:
